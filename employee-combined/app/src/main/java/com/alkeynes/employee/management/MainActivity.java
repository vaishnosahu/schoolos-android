...
public class MainActivity extends Activity {
    ...
    private void ensureEmployeePermissions(boolean startWhenReady){SharedPreferences sp=getSharedPreferences(PREFS,MODE_PRIVATE);if(!"employee".equals(currentRole)&&sp.getInt("employee_id",0)<=0)return;if(Build.VERSION.SDK_INT>=33&&checkSelfPermission(Manifest.permission.POST_NOTIFICATIONS)!=PackageManager.PERMISSION_GRANTED){if(!sp.getBoolean("shown_notification_guide",false)){sp.edit().putBoolean("shown_notification_guide",true).apply();new AlertDialog.Builder(this).setTitle("Allow tracking notifications").setMessage("While you are clocked in, Employee Management shows an ongoing notification so you can see that work-location tracking is active.").setPositiveButton("Continue",(d,w)->requestPermissions(new String[]{Manifest.permission.POST_NOTIFICATIONS},REQ_NOTIFICATIONS)).setNegativeButton("Not now",null).show();}else requestPermissions(new String[]{Manifest.permission.POST_NOTIFICATIONS},REQ_NOTIFICATIONS);}
        if(checkSelfPermission(Manifest.permission.ACCESS_FINE_LOCATION)!=PackageManager.PERMISSION_GRANTED){pendingStart=startWhenReady;if(!sp.getBoolean("shown_location_guide",false)){sp.edit().putBoolean("shown_location_guide",true).apply();new AlertDialog.Builder(this).setTitle("Location for attendance").setMessage("Employee Management uses precise location for attendance and, while your active work session is running, for authorised live tracking. Tracking is stopped when the server says your work session or permitted shift is no longer active.").setPositiveButton("Continue",(d,w)->requestPermissions(new String[]{Manifest.permission.ACCESS_FINE_LOCATION,Manifest.permission.ACCESS_COARSE_LOCATION},REQ_LOCATION)).setNegativeButton("Not now",null).show();}else requestPermissions(new String[]{Manifest.permission.ACCESS_FINE_LOCATION,Manifest.permission.ACCESS_COARSE_LOCATION},REQ_LOCATION);return;}
        if(startWhenReady)startTrackingService();maybeGuideBackgroundLocation();}
    ...
    @Override public void onRequestPermissionsResult(int requestCode,String[] permissions,int[] grantResults){super.onRequestPermissionsResult(requestCode,permissions,grantResults);if(requestCode==REQ_LOCATION&&grantResults.length>0&&grantResults[0]==PackageManager.PERMISSION_GRANTED){if(pendingStart)startTrackingService();webView.reload();maybeGuideBackgroundLocation();}}
